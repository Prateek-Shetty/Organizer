...
// Load API keys securely (if available)
const API_KEY = process.env.GEMINI_API_KEY;
const YOUTUBE_API_KEY = process.env.YOUTUBE_API_KEY;
const GOOGLE_BOOKS_API_KEY = process.env.GOOGLE_BOOKS_API_KEY;
...
// ✅ Fetch YouTube Videos (if API key is provided)
if (YOUTUBE_API_KEY) {
    app.get("/youtube", async (req, res) => {
        try {
            const query = req.query.query;
...
// ✅ Fetch Books from Google Books API (if API key is provided)
...
// ✅ Recognize Handwritten Text (Local Processing - Simulated)
app.post("/recognize-text", (req, res) => {
    const { text } = req.body;
    const recognizedText = text.toUpperCase(); // Simulated recognition
